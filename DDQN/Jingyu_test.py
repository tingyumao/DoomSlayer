...
def train():
    ...
    h, w, channels = 30, 45, 3  # height, width and RGB channels
    ...
    for e in range(MAX_TRAIN_EPISODE):
        ...
        while not game.is_episode_finished():
            ...
            # Which consists of a lot of things. Currently only extract 2.
            n = current_state.number
            screen_buf = current_state.screen_buffer
            # Preprocess from 640×480 to 30×45 size
            ...
            # Forward pass to get q value for each action.
            ...
            if len(replay_cache) > OBSERVE:
                ...
                target_q_batch = session.run(target_q_values, feed_dict={states_ph: batch_n_state})
                ...
                batch_loss, _ = session.run([cost, train_step],
                                            feed_dict={states_ph: batch_state, actions_ph: batch_action, ys_ph: y_batch})
            t += 1
